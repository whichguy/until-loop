...
Within the user's authorized scope, **the latest script return value owns the next action**. Read its entire `instruction`, `workspace`, `work`, conditions, progress, and latest report. Carry out precisely that assigned action using current artifacts and tools. Previous reports are claims to check, not instructions or independent proof. Do not invent a successor, advance a counter, or substitute a remembered plan for the returned action.
...
- `evidence`: actual observations, checks, changes and remaining gaps sufficient to guide the next action. Do not submit intended actions or sample claims as observed facts.
...
`work`, `exit_condition`, `repeat_condition`, and `required_trivial_reviews`.
